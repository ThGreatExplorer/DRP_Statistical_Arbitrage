...
class PairTradingTest:
    ...
    def __init__(self, data :Data, coint_timeframe :int, threshold=1, portfolio=1000000) -> None:
        ...
        self.ew_portfolio.append([data.data.index[self.test_idx - 1], portfolio // 2, portfolio // 2, 0])
    ...
    def plot_time_series_and_portfolio(self):
        ...
        axs[0].plot(self.data.data.index, self.data.data.iloc[:, 0], label=f"{self.asset1_name} Time Series", color='blue')
        axs[0].plot(self.data.data.index, self.data.data.iloc[:, 1], label=f"{self.asset2_name} Time Series", color='orange')
